...
const API_BASE =
  import.meta.env.VITE_API_BASE_URL?.replace("/api/sites", "") ||
  "http://localhost:5000";
...
function ErrorBanner({ error, onDismiss }) {
  ...
  const messages = {
    repo_not_found:
      "Repository not found. Double-check the owner/repo spelling.",
    repo_private_or_forbidden:
      "Repository is private or access is forbidden. Provide a GitHub token (PAT) with repo read scope.",
    ...
    no_endpoints_detected:
      "No API endpoints were detected. This repo may not use a supported framework (Express, Next.js, FastAPI) or have an OpenAPI/Postman spec.",
    invalid_input:
      "Invalid input. Use the format: owner/repo (e.g. goraiabhijit/uptime_sentinal)",
  };
  const msg =
    messages[error.code] || error.message || "An unexpected error occurred.";
  return (
    <div
      style={{
    ...
    >
      ...
      />
      <p style={{ color: "#fca5a5", fontSize: "0.85rem", margin: 0, flex: 1 }}>
        {msg}
      </p>
      <button
        onClick={onDismiss}
...
export default function ScanModal({ onClose, onSaved }) {
  ...
  const toggleEndpoint = (id) =>
    setEndpoints((eps) =>
      eps.map((ep) => (ep.id === id ? { ...ep, selected: !ep.selected } : ep)),
    );
  ...
  const updateParam = (id, param, value) =>
    setEndpoints((eps) =>
      eps.map((ep) =>
        ep.id === id
          ? { ...ep, paramOverrides: { ...ep.paramOverrides, [param]: value } }
          : ep,
      ),
    );
    ...
    try {
      ...
      const toSave = endpoints
        ...
        .map((ep) => ({
          method: ep.method,
          path: ep.path,
          pathParamOverrides: ep.paramOverrides || {},
          headers: {},
        }));
    ...
    >
      <div style={s.modal}>
        ...
        <div style={s.body}>
          ...
          {step === 1 && (
            <div>
              ...
              >
                Enter a GitHub repository and Uptime Sentinel will automatically
                discover its API endpoints from specs, routing conventions, or
                source code.
              </p>
              <ErrorBanner
                error={scanError}
              ...
              <div style={{ position: "relative", marginBottom: "1rem" }}>
                ...
                <input
                  ...
                  onKeyDown={(e) =>
                    e.key === "Enter" && repoInput.trim() && handleScan()
                  }
                  autoFocus
                />
              </div>
              ...
              >
                GitHub Token (Optional — raises rate limit from 60 → 5,000
                req/hr)
              </label>
              ...
              >
                <p style={{ color: "#64748b", fontSize: "0.78rem", margin: 0 }}>
                  ...
                  OpenAPI/Swagger specs · Postman collections · Next.js (App +
                  Pages Router) · Express routes · FastAPI routers · Source code
                  regex fallback
                </p>
              </div>
            </div>
            ...
            <div>
              ...
              >
                ...
                <p style={{ color: "#4ade80", fontSize: "0.85rem", margin: 0 }}>
                  Found <strong>{scanResult.endpoints.length}</strong> endpoint
                  {scanResult.endpoints.length !== 1 ? "s" : ""} on branch{" "}
                  <code
                    style={{
                      backgroundColor: "#0f172a",
                      padding: "1px 6px",
                  ...
                  {scanResult.truncated && (
                    <span style={{ color: "#fbbf24" }}>
                      {" "}
                      (tree was truncated — results may be incomplete)
                    </span>
                  )}
                </p>
              </div>
              ...
              <div style={{ position: "relative", marginBottom: "0.5rem" }}>
                ...
                <input
                  ...
                  style={{
                    ...
                    paddingLeft: "2.25rem",
                    borderColor:
                      baseUrl && !baseUrlValid ? "#ef4444" : "#334155",
                  }}
              ...
              >
                This will be prepended to each discovered path, e.g.{" "}
            ...
            <div>
              ...
              >
                ...
                >
                  <span style={{ color: "#94a3b8", fontSize: "0.8rem" }}>
                    {selectedCount}/{endpoints.length} selected
                  </span>
                  <button onClick={selectAll} style={s.btn("secondary")}>
                    All
                  </button>
                  <button onClick={selectNone} style={s.btn("secondary")}>
                    None
                  </button>
                </div>
                <div
              ...
              {saveResult && !saveResult.error && (
                ...
                >
                  ...
                  >
                    ✅ Saved {saveResult.created} new + {saveResult.updated}{" "}
                    updated monitors. Initial health checks are running in the
                    background.
                  </p>
                </div>
              )}
              {saveResult?.error && (
                ...
                >
                  <p
                    style={{ color: "#fca5a5", fontSize: "0.85rem", margin: 0 }}
                  >
                    ❌ {saveResult.error}
                  </p>
                </div>
                ...
                >
                  <thead>
                    ...
                    >
                      ...
                      <th style={{ padding: "0.6rem 0.5rem", width: 70 }}>
                        Method
                      </th>
                      <th style={{ padding: "0.6rem 0.5rem" }}>Path</th>
                      <th style={{ padding: "0.6rem 0.5rem", width: 80 }}>
                        Confidence
                      </th>
                          ...
                          >
                            ...
                            <td
                              style={{
                                ...
                                fontFamily: "monospace",
                                color: ep.selected ? "#f8fafc" : "#64748b",
                              }}
                            >
                              {ep.path}
                            </td>
                            ...
                            >
                              <button
                                onClick={(e) => {
                                  e.stopPropagation();
                                  setExpandedSource(isExpanded ? null : ep.id);
                                }}
                                style={{
                                  background: "none",
                                  border: "none",
                              ...
                              >
                                <p
                                  style={{
                                    ...
                                    fontSize: "0.75rem",
                                    fontFamily: "monospace",
                                    marginBottom: params.length ? "0.5rem" : 0,
                                  }}
                                >
                                  📄 {ep.sourceFile}
                                  {ep.sourceLine ? `:${ep.sourceLine}` : ""}
                                </p>
                                {params.length > 0 && (
                                  <div
                                    style={{
              ...
              {scanResult?.duplicatesFound?.length > 0 && (
                ...
                >
                  ℹ {scanResult.duplicatesFound.length} duplicate endpoint
                  {scanResult.duplicatesFound.length !== 1 ? "s" : ""} were
                  deduplicated (highest-confidence source kept).
                </p>
              )}
            </div>
          )}
        ...
        <div style={s.footer}>
          {step === 1 && (
            <>
              <button onClick={onClose} style={s.btn("secondary")}>
                Cancel
              </button>
              <button
                id="scan-repo-btn"
          ...
          {step === 2 && (
            <>
              <button onClick={() => setStep(1)} style={s.btn("secondary")}>
                ← Back
              </button>
              <button
                id="scan-next-btn"
          ...
          {step === 3 && (
            <>
              ...
              <button
                ...
                onClick={handleSave}
                disabled={
                  selectedCount === 0 || saving || !!saveResult?.monitorIds
                }
                style={s.btn(
                  "primary",
                  selectedCount === 0 || saving || !!saveResult?.monitorIds,
